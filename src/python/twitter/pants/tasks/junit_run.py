...
class JUnitRun(JvmTask):
  ...
  def __init__(self, context):
    ...
    self._bootstrap_tools = context.config.getlist('junit-run', 'bootstrap-tools', default=[':junit'])
    self._emma_bootstrap_tools = context.config.get('junit-run', 'emma-bootstrap-tools', default=[':emma'])
  ...
  def execute(self, targets):
    if not self.context.options.junit_run_skip:
      ...
      if tests:
        ...
        if self.coverage:
          emma_classpath = self.bootstrap_classpath(self._emma_bootstrap_tools)
