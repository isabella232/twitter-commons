...
from twitter.common.dirutil import safe_rmtree, safe_mkdir, safe_open, safe_delete
...
from twitter.pants.tasks.jvm_compile.classpath import Directory, FilteredClassPathElement
...
class JvmCompile(NailgunTask):
  ...
  def setup_parser(subcls, option_group, args, mkflag):
    ...
    option_group.add_option(mkflag('args'),
                            dest=subcls._language+'_compile_args',
                            action='append',
  ...
  def __init__(self, context, minimum_version=None, jdk=False):
    ...
    self._upstream_classes_file = os.path.join(workdir, 'upstream_classes')
    safe_delete(self._upstream_classes_file)
    ...
    def wrap(args):
      if not args:
        return []
      return ['%s%s' % (concrete_class._compiler_arg_prefix, x) for x in args]

    self._args = wrap(context.config.getlist(config_section, 'args'))
    if get_lang_specific_option('compile_warnings'):
      self._args.extend(wrap(context.config.getlist(config_section, 'warning_args')))
    else:
      self._args.extend(wrap(context.config.getlist(config_section, 'no_warning_args')))
    for arg in get_lang_specific_option('compile_args') or []:
      self._args.extend(wrap(shlex.split(arg)))
  ...
  def execute(self, targets):
    ...
    # Add output and resource dirs to the classpath for us and for downstream tasks.
    egroups.add_to_compatible_classpaths(group_id, Directory(self._resources_dir))
    egroups.add_to_compatible_classpaths(group_id,
        FilteredClassPathElement(Directory(self._classes_dir),
                                 whitelist_path=self._upstream_classes_file))
  ...
  def _process_target_partition(self, partition, classpath):
    ...
    classpath - a ClassPath instances.
    ...
    if not sources:
      ...
      with self.context.new_workunit('compile'):
        ...
        self.compile(classpath, sources, self._classes_dir, analysis_file)
  ...
  def _register_products(self, targets, sources_by_target, analysis_file):
    ...
    def to_binary_name(path):
      if not path.endswith('.class'):
        return path
      return os.path.relpath(path[:-6], self._classes_dir).replace(os.path.sep, '.')
    ...
    for target in targets:
      ...
      for source in sources_by_target[target]:  # Source is relative to buildroot.
        ...
        with safe_open(self._upstream_classes_file, 'a') as outfile:
          for cls in classes:
            outfile.write(to_binary_name(cls))
            outfile.write('\n')
