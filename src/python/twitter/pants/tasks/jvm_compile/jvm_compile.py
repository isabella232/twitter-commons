...
class JvmCompile(NailgunTask):
  ...
  def __init__(self, context, minimum_version=None, jdk=False):
    ...
    self._classes_jar = os.path.join(workdir, 'classes.jar')
    ...
    # Valid targets whose products we've registered.
    self._registered_targets = set()
  ...
  def execute(self, targets):
    ...
    # Add resource dirs and classes jar to the classpath for us and for downstream tasks.
    ...
    if self._class_to_jarfile is None:
      ...
      for jarpath in self.find_all_bootstrap_jars() + classpath_jars:
        ...
        if os.path.isfile(jarpath) and ((jarpath.endswith('.jar') or jarpath.endswith('.zip'))):
          with open_jar(jarpath, 'r') as jar:
  ...
  def _register_products(self, targets, sources_by_target, analysis_file):
    ...
    for target in targets:
      # This check is necessary: multiple chunks might attempt to register the
      # same valid upstream targets.
      if target in self._registered_targets:
        continue
      ...
      self._append_to_target_classes_jar(target_products)
      self._registered_targets.add(target)

  def _append_to_target_classes_jar(self, products):
    """Create a jar containing the target's classfiles.

    This jar is put on the upstream classpath when compiling things that depen on this target,
    so that the compiler can see these classes. This is preferable to putting self._classes_dir
    on the classpath. That has all sorts of unintended consequences.
    """
    with self.context.new_workunit(name='update-upstream-jar'):
      with open_jar(self._classes_jar, 'a', compression=ZIP_DEFLATED) as jar:
        for root, classes in products.rel_paths():
          for cls in classes:
            jar.write(os.path.join(root, cls), cls)
